...
BEGIN_MESSAGE_MAP(InMoneyDlg, CDialogEx)
	ON_BN_CLICKED(IDC_BUTTON_INMONEY_CANCEL, &InMoneyDlg::OnBnClickedButtonInmoneyCancel)//映射“单击取消按钮”事件
	ON_BN_CLICKED(IDC_BUTTON_INMONEY_OK, &InMoneyDlg::OnClickedButtonInmoneyOk)//映射“单击确定按钮”事件
END_MESSAGE_MAP()
//本对话框内的事件映射结束


InMoneyDlg::InMoneyDlg(CWnd* pParent /*=nullptr*/)//	“输入储蓄金额”窗口的构造方法，参数为空则父窗口指针为空
	: CDialogEx(IDD_INMONEY, pParent)//		为“输入储蓄金额”窗口绑定资源
{
	m_hIcon = AfxGetApp()->LoadIcon(IDI_ICON1);//为“输入储蓄金额”窗口添加图标
}

BOOL InMoneyDlg::OnInitDialog() // 当对话框初始化时执行此函数
{
...
void InMoneyDlg::OnBnClickedButtonInmoneyCancel()//点击取消按钮时执行此函数
{
	this->SendMessage(WM_CLOSE);//向本窗口发送“关闭窗口”消息
}


void InMoneyDlg::OnClickedButtonInmoneyOk()//当点击“确认存入”按钮之后执行此函数
{
	//开始获取输入框内金钱数量
	CEdit* moneyEdit;//创建输入框类型指针
	moneyEdit = (CEdit*)GetDlgItem(IDC_EDIT_INMONEY_NUMBER);//将输入框类型指针指向输入框
	...
	if ((moneyEditText.Compare(_T("")) == 0)) {//如果输入为空
		...
		if (!(moneyEditText == moneyEditText.SpanIncluding(_T("0123456789.")))) {//如果输入的内容有除了数字和小数点以外的字符
			...
			delete moneyArr;//删除用来存储所有用户金钱的数组
			MessageBox(_T("存入：") + moneyEditText + _T("  RMB(CNY)"), _T("存入成功"), MB_OK);//弹出对话框（参数1：对话框内容，参数2：对话框标题，参数3：确定按钮）
			...
			UserDataDlg userDataDlg;//创建“用户界面”的窗口
